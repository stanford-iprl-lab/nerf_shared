...
def run():
    ...
    my_nerf = NeRF(args)
    renderer = Render(my_nerf, 400, 400, fov=50) #50mm

    train_utils = TrainUtils(images, poses)

    train_utils.create_optimizer(my_nerf)
    ...
    if args.custom is False:
        ...
        for i in trange(start, N_iters):
            ...
            rays, poses, gt_images = train_utils.get_train_batch()

            optimizer.zero_grad()
            rgb_pixels = renderer.get_subsambled_img(rays, poses)

            loss = train_utils.loss( rgb_pixels, rays, gt_images )

            loss.backward()
            train_utils.optimizer.step()

            train_utils.optimizer.update()
