...
class state_transform(nn.Module):
    def __init__(self):
        ...
        self.vel = nn.Parameter(torch.normal(0., 1e-3, size=(3,)))

        self.w = nn.Parameter(torch.normal(0., 1e-3, size=(3,)))
        self.v = nn.Parameter(torch.normal(0., 1e-3, size=(3,)))

        self.omega = nn.Parameter(torch.normal(0., 1e-3, size=(3,)))

    def forward(self, x):
        #x is initial estimate on the 18 dimensional state
        '''
        # Converts 12-dimensional representation to 18 dimensional (position, velocity, 9 elements of rotation matrix, ang. velocity)
        T = torch.zeros((18,))
        T[:3] = self.pos
        T[3:6] = self.vel
        T[15:18] = self.omega

        psi = self.angle[0]     #yaw
        theta = self.angle[1]   #pitch
        phi = self.angle[2]     #roll

        R = torch.zeros((9, ))
        R[0] = torch.cos(theta)*torch.cos(psi)
        R[1] = torch.sin(phi)*torch.sin(theta)*torch.cos(psi) - torch.cos(phi)*torch.sin(psi)
        R[2] = torch.cos(phi)*torch.sin(theta)*torch.cos(psi) + torch.sin(phi)*torch.sin(psi)
        R[3] = torch.cos(theta)*torch.sin(psi)
        R[4] = torch.sin(phi)*torch.sin(theta)*torch.sin(psi) + torch.cos(phi)*torch.cos(psi)
        R[5] = torch.cos(phi)*torch.sin(theta)*torch.sin(psi) - torch.sin(phi)*torch.cos(psi)
        R[6] = -torch.sin(theta)
        R[7] = torch.sin(phi)*torch.cos(theta)
        R[8] = torch.cos(phi)*torch.cos(theta)
        '''
        ...
        exp_i[:3, 3] = torch.matmul(torch.eye(3) * theta + (1 - torch.cos(theta)) * w_skewsym + (theta - torch.sin(theta)) * torch.matmul(w_skewsym, w_skewsym), self.v)
        exp_i[3, 3] = 1.
...
class Estimator():
    ...
    def optimize(self, start_state, sig, obs_img, obs_img_pose, obs_img_depth=None):
        ...
        for k in range(self.iter):
            ...
            #Process loss. 
            loss_dyn = quad_loss(predict_state, start_state, sig)
            ...
            if TEST == True:
                ...
                loss = loss_rgb + loss_depth + loss_rgb_fine + loss_depth_fine + loss_dyn
    ...
    def measurement_function(self, state, start_state, sig, obs_img, obs_img_pose, obs_img_depth=None):
        ...
        #print('Loss', loss, 'RGB Loss', loss_rgb, 'Process Loss', loss_dyn)

        return loss

    def estimate_state(self, obs_img, obs_img_pose, obs_img_depth=None, action=None):
        ...
        xt = self.optimize(torch.tensor(start_state), torch.tensor(sig_prop), obs_img, obs_img_pose, obs_img_depth)
        ...
        t3 = time.time()
        sig = torch.autograd.functional.hessian(lambda x: self.measurement_function(x, start_state, sig_prop, obs_img, obs_img_pose, obs_img_depth), self.xt)
        t4 = time.time()
